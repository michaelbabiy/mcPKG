//
//  MCUpdateSubmissionViewController.m
//  mcPKG
//
...
#import "MCUpdateSubmissionViewController.h"
#import "MCDescriptionViewController.h"
#import "MCRawControl.h"

@interface MCUpdateSubmissionViewController () <UISearchBarDelegate>
...
@implementation MCUpdateSubmissionViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self setFetchedResultsControllerForEntity:kMCSubmissionEntityName
                                     predicate:[NSPredicate predicateWithFormat:@"submissionFormID == %@ && submissionNew == %@", kJotFormUpdatePackageSubmissionForm, @YES]
                                          sort:@"submissionDate"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"MCDescriptionViewController"]) {
        ...
        self.rawControl = [[MCRawControl alloc]initWithName:submission.packageName
                                                description:submission.packageChangeLog
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if ([searchText length] > 0) {
        searchPredicate = [NSPredicate predicateWithFormat:@"packageName CONTAINS [cd] %@ && submissionFormID == %@", searchText, kJotFormUpdatePackageSubmissionForm];
    } else {
        searchPredicate = [NSPredicate predicateWithFormat:@"submissionFormID == %@", kJotFormUpdatePackageSubmissionForm];
    }
